...
char		g_szLog[MAX_PATH]			= {0, };
char		g_szTsString[MAX_PATH]		= {0, };
char		g_szTsTimeString[MAX_PATH]	= {0, };
...
#ifdef _MSC_VER
static AVRational make_time_base_q(int num, int den)
{
	AVRational TimeBaseQ = {num, den};
	return TimeBaseQ;
}
#endif
    ...
    if (vstats_file) {
        if (fclose(vstats_file))
            av_log(NULL, AV_LOG_ERROR,
                   ...
                   av_make_error_string(g_szLog, MAX_PATH, AVERROR(errno)));
    }
...
#ifdef _MSC_VER
		OSTFinished _Finished = (OSTFinished)(ost2->finished | ost->finished);
        _Finished == ost2->finished ? this_stream : others;
#else
...
static void write_frame(AVFormatContext *s, AVPacket *pkt, OutputStream *ost)
{
    ...
    if (debug_ts) {
        av_log(NULL, AV_LOG_INFO, "muxer <- type:%s "
                ...
                av_ts_make_string(g_szTsString, pkt->pts), av_ts_make_time_string(g_szTsTimeString, pkt->pts, &ost->st->time_base),
                av_ts_make_string(g_szTsString, pkt->dts), av_ts_make_time_string(g_szTsTimeString, pkt->dts, &ost->st->time_base),
                pkt->size
              );
    }
...
#ifdef _MSC_VER
	OSTFinished _Finished = (OSTFinished)(ost->finished | ENCODER_FINISHED);
    ost->finished = _Finished;
#else
    ost->finished |= ENCODER_FINISHED;
#endif

    if (of->shortest) {
        int64_t end = av_rescale_q(ost->sync_opts - ost->first_pts, ost->enc_ctx->time_base, g_TimeBaseQ);
                         ...
                         AVFrame *frame)
{
    ...
    if (debug_ts) {
        av_log(NULL, AV_LOG_INFO, "encoder <- type:audio "
               ...
               av_ts_make_string(g_szTsString, frame->pts), av_ts_make_time_string(g_szTsTimeString, frame->pts, &enc->time_base),
                            ...
                            AVSubtitle *sub)
{
    ...
    for (i = 0; i < nb; i++) {
        ...
        ost->sync_opts = av_rescale_q(pts, g_TimeBaseQ, enc->time_base);
        if (!check_recording_time(ost))
            return;

        sub->pts = pts;
        // start_display_time is required to be 0
        sub->pts               += av_rescale_q(sub->start_display_time, make_time_base_q( 1, 1000 ), g_TimeBaseQ);
        ...
        pkt.pts  = av_rescale_q(sub->pts, g_TimeBaseQ, ost->st->time_base);
        pkt.duration = av_rescale_q(sub->end_display_time, make_time_base_q( 1, 1000 ), ost->st->time_base);
        if (enc->codec_id == AV_CODEC_ID_DVB_SUBTITLE) {
...
#endif
    {
        ...
        if (debug_ts) {
            av_log(NULL, AV_LOG_INFO, "encoder <- type:video "
                   "frame_pts:%s frame_pts_time:%s time_base:%d/%d\n",
                   av_ts_make_string(g_szTsString, in_picture->pts), av_ts_make_time_string(g_szTsTimeString, in_picture->pts, &enc->time_base),
                   enc->time_base.num, enc->time_base.den);
        }
        ...
        if (got_packet) {
            if (debug_ts) {
                av_log(NULL, AV_LOG_INFO, "encoder -> type:video "
                       "pkt_pts:%s pkt_pts_time:%s pkt_dts:%s pkt_dts_time:%s\n",
                       av_ts_make_string(g_szTsString, pkt.pts), av_ts_make_time_string(g_szTsTimeString, pkt.pts, &enc->time_base),
                       av_ts_make_string(g_szTsString, pkt.dts), av_ts_make_time_string(g_szTsTimeString, pkt.dts, &enc->time_base));
            }
        ...
        while (1) {
            ...
            if (ret < 0) {
                if (ret != AVERROR(EAGAIN) && ret != AVERROR_EOF) {
                    av_log(NULL, AV_LOG_WARNING,
                           "Error in av_buffersink_get_frame_flags(): %s\n", av_make_error_string(g_szLog, MAX_PATH, ret));
            ...
            if (filtered_frame->pts != AV_NOPTS_VALUE) {
                ...
                float_pts =
                    ...
                    av_rescale_q(start_time, g_TimeBaseQ, tb);
                ...
                filtered_frame->pts =
                    ...
                    av_rescale_q(start_time, g_TimeBaseQ, enc->time_base);
            }
            //if (ost->source_index >= 0)
            ...
            case AVMEDIA_TYPE_VIDEO:
                ...
                if (debug_ts) {
                    av_log(NULL, AV_LOG_INFO, "filter -> pts:%s pts_time:%s exact:%f time_base:%d/%d\n",
                            av_ts_make_string(g_szTsString, filtered_frame->pts), av_ts_make_time_string(g_szTsTimeString, filtered_frame->pts, &enc->time_base),
                            float_pts,
                            enc->time_base.num, enc->time_base.den);
                }
...
static void do_streamcopy(InputStream *ist, OutputStream *ost, const AVPacket *pkt)
{
    ...
    int64_t start_time = (of->start_time == AV_NOPTS_VALUE) ? 0 : of->start_time;
    int64_t ost_tb_start_time = av_rescale_q(start_time, g_TimeBaseQ, ost->st->time_base);
    AVPicture pict;
    AVPacket opkt;
    ...
    if (!ost->frame_number && !ost->copy_prior_start) {
        ...
        if (pkt->pts == AV_NOPTS_VALUE ?
            ist->pts < comp_start :
            pkt->pts < av_rescale_q(comp_start, g_TimeBaseQ, ist->st->time_base))
            return;
    }
    ...
    if (pkt->dts == AV_NOPTS_VALUE)
        opkt.dts = av_rescale_q(ist->dts, g_TimeBaseQ, ost->st->time_base);
    else
    ...
    if (ost->st->codec->codec_type == AVMEDIA_TYPE_AUDIO && pkt->dts != AV_NOPTS_VALUE) {
        ...
        opkt.dts = opkt.pts = av_rescale_delta(ist->st->time_base, pkt->dts,
                                               make_time_base_q(1, ist->dec_ctx->sample_rate), duration, &ist->filter_in_rescale_delta_last,
...
static int decode_audio(InputStream *ist, AVPacket *pkt, int *got_output)
{
    ...
    if (decoded_frame->pts != AV_NOPTS_VALUE) {
        ist->dts = ist->next_dts = ist->pts = ist->next_pts = av_rescale_q(decoded_frame->pts, avctx->time_base, g_TimeBaseQ);
        ...
        decoded_frame_tb   = g_TimeBaseQ;
    }
    pkt->pts           = AV_NOPTS_VALUE;
    if (decoded_frame->pts != AV_NOPTS_VALUE)
        decoded_frame->pts = av_rescale_delta(decoded_frame_tb, decoded_frame->pts,
                                              make_time_base_q(1, avctx->sample_rate), decoded_frame->nb_samples, &ist->filter_in_rescale_delta_last,
                                              make_time_base_q(1, avctx->sample_rate));
...
static int decode_video(InputStream *ist, AVPacket *pkt, int *got_output)
{
    ...
    if (!ist->filter_frame && !(ist->filter_frame = av_frame_alloc()))
        return AVERROR(ENOMEM);
    decoded_frame = ist->decoded_frame;
    pkt->dts  = av_rescale_q(ist->dts, g_TimeBaseQ, ist->st->time_base);
    ...
    if(best_effort_timestamp != AV_NOPTS_VALUE) {
        int64_t ts = av_rescale_q(decoded_frame->pts = best_effort_timestamp, ist->st->time_base, g_TimeBaseQ);
    ...
    if (debug_ts) {
        av_log(NULL, AV_LOG_INFO, "decoder -> ist_index:%d type:video "
               ...
               ist->st->index, av_ts_make_string(g_szTsString, decoded_frame->pts),
               av_ts_make_time_string(g_szTsTimeString, decoded_frame->pts, &ist->st->time_base),
               best_effort_timestamp,
               av_ts_make_time_string(g_szTsTimeString, best_effort_timestamp, &ist->st->time_base),
...
static int process_input_packet(InputStream *ist, const AVPacket *pkt, int no_eof)
{
    ...
    if (!ist->saw_first_ts) {
        ...
        if (pkt && pkt->pts != AV_NOPTS_VALUE && !ist->decoding_needed) {
            ist->dts += av_rescale_q(pkt->pts, ist->st->time_base, g_TimeBaseQ);
    ...
    if (pkt->dts != AV_NOPTS_VALUE) {
        ist->next_dts = ist->dts = av_rescale_q(pkt->dts, ist->st->time_base, g_TimeBaseQ);
    ...
    handle_eof:
        ...
        case AVMEDIA_TYPE_VIDEO:
            ...
            if (avpkt.duration) {
                duration = av_rescale_q(avpkt.duration, ist->st->time_base, g_TimeBaseQ);
        ...
        if (ret < 0) {
            av_log(NULL, AV_LOG_ERROR, "Error while decoding stream #%d:%d: %s\n",
                   ist->file_index, ist->st->index, av_make_error_string(g_szLog, MAX_PATH, ret));
            if (exit_on_error)
                exit_program(1);
            break;
        }
    ...
    if (!ist->decoding_needed) {
        ...
        case AVMEDIA_TYPE_VIDEO:
            if (ist->framerate.num) {
                ...
                AVRational time_base_q = g_TimeBaseQ;
                int64_t next_dts = av_rescale_q(ist->next_dts, time_base_q, av_inv_q(ist->framerate));
                ist->next_dts = av_rescale_q(next_dts + 1, av_inv_q(ist->framerate), time_base_q);
            } else if (pkt->duration) {
                ist->next_dts += av_rescale_q(pkt->duration, ist->st->time_base, g_TimeBaseQ);
...
static int init_output_stream(OutputStream *ost, char *error, int error_len)
{
    ...
    if (ost->encoding_needed) {
        ...
        if (ost->enc_ctx->nb_coded_side_data) {
            ...
            ost->st->side_data = (AVPacketSideData *)av_realloc_array(NULL, ost->enc_ctx->nb_coded_side_data,
        ...
        ost->st->time_base = av_add_q(ost->enc_ctx->time_base, make_time_base_q(0, 1));
        ...
        // copy timebase while removing common factors
        ost->st->time_base = av_add_q(ost->st->codec->time_base, make_time_base_q(0, 1));
    }
                                    ...
                                    AVCodecContext *avctx)
{
    ...
    for (i = 0; i < n; i++) {
        ...
        if (!memcmp(p, "chapters", 8)) {
            ...
            t = av_rescale_q(t, g_TimeBaseQ, avctx->time_base);
            ...
            av_assert1(index < size);
            pts[index++] = av_rescale_q(t, g_TimeBaseQ, avctx->time_base);
...
static void report_new_stream(int input_index, AVPacket *pkt)
{
    ...
    av_log(file->ctx, AV_LOG_WARNING,
           ...
           pkt->pos, av_ts_make_time_string(g_szTsTimeString, pkt->dts, &st->time_base));
    ...
    for (i = 0; i < nb_output_streams; i++) {
        ...
        if (ost->stream_copy) {
                ...
                if (ost->frame_aspect_ratio.num) { // overridden by the -aspect cli option
                    sar =
                        av_mul_q(ost->frame_aspect_ratio,
                                 make_time_base_q( enc_ctx->height, enc_ctx->width ));
            ...
            if (enc_ctx->codec_type == AVMEDIA_TYPE_VIDEO) {
                ...
                if (ist && !ost->frame_rate.num) {
                    ost->frame_rate = make_time_base_q(25, 1);
                    av_log(NULL, AV_LOG_WARNING,
                           "No information "
...
//                    ost->frame_rate = ist->st->avg_frame_rate.num ? ist->st->avg_frame_rate : make_time_base_q(25, 1);
            ...
            switch (enc_ctx->codec_type) {
            case AVMEDIA_TYPE_AUDIO:
                enc_ctx->sample_fmt     = (AVSampleFormat)ost->filter->filter->inputs[0]->format;
                enc_ctx->sample_rate    = ost->filter->filter->inputs[0]->sample_rate;
                enc_ctx->channel_layout = ost->filter->filter->inputs[0]->channel_layout;
                enc_ctx->channels       = avfilter_link_get_channels(ost->filter->filter->inputs[0]);
                enc_ctx->time_base      = make_time_base_q( 1, enc_ctx->sample_rate );
                break;
            case AVMEDIA_TYPE_VIDEO:
                enc_ctx->time_base = av_inv_q(ost->frame_rate);
                ...
                enc_ctx->sample_aspect_ratio = ost->st->sample_aspect_ratio =
                    ...
                    av_mul_q(ost->frame_aspect_ratio, make_time_base_q( enc_ctx->height, enc_ctx->width )) :
                ...
                break;
            case AVMEDIA_TYPE_SUBTITLE:
                enc_ctx->time_base = make_time_base_q(1, 1000);
                if (!enc_ctx->width) {
        ...
        if (ost->disposition) {           
			...
			for (int Index = 1; Index < sizeof(opts) / sizeof(*opts) - 1; ++Index)
			{
				opts[Index].default_val.dbl	= .0;
				opts[Index].default_val.str	= NULL;
				memset(&opts[Index].default_val.q, 0, sizeof(AVRational));
			}
    ...
    for (i = 0; i < nb_output_files; i++) {
        ...
        if ((ret = avformat_write_header(oc, &output_files[i]->opts)) < 0) {
            snprintf(error, sizeof(error),
                     ...
                     i, av_make_error_string(g_szLog, MAX_PATH, ret));
            ret = AVERROR(EINVAL);
            goto dump_format;
        }
...
static int process_input(int file_index)
{
    ...
    if (debug_ts) {
        av_log(NULL, AV_LOG_INFO, "demuxer -> ist_index:%d type:%s "
               ...
               av_ts_make_string(g_szTsString, ist->next_dts), av_ts_make_time_string(g_szTsTimeString, ist->next_dts, &g_TimeBaseQ),
               av_ts_make_string(g_szTsString, ist->next_pts), av_ts_make_time_string(g_szTsTimeString, ist->next_pts, &g_TimeBaseQ),
               av_ts_make_string(g_szTsString, pkt.pts), av_ts_make_time_string(g_szTsTimeString, pkt.pts, &ist->st->time_base),
               av_ts_make_string(g_szTsString, pkt.dts), av_ts_make_time_string(g_szTsTimeString, pkt.dts, &ist->st->time_base),
               av_ts_make_string(g_szTsString, input_files[ist->file_index]->ts_offset),
               av_ts_make_time_string(g_szTsTimeString, input_files[ist->file_index]->ts_offset, &g_TimeBaseQ));
    }
    ...
    if(!ist->wrap_correction_done && is->start_time != AV_NOPTS_VALUE && ist->st->pts_wrap_bits < 64){
        ...
        if (   ist->next_dts == AV_NOPTS_VALUE
            ...
            for (i=0; i<is->nb_streams; i++) {
                ...
                if(st->discard == AVDISCARD_ALL || st->start_time == AV_NOPTS_VALUE)
                    continue;
                new_start_time = FFMIN(new_start_time, av_rescale_q(st->start_time, st->time_base, g_TimeBaseQ));
            }
        ...
        stime = av_rescale_q(is->start_time, g_TimeBaseQ, ist->st->time_base);
    ...
    if (pkt.dts != AV_NOPTS_VALUE)
        pkt.dts += av_rescale_q(ifile->ts_offset, g_TimeBaseQ, ist->st->time_base);
    if (pkt.pts != AV_NOPTS_VALUE)
        pkt.pts += av_rescale_q(ifile->ts_offset, g_TimeBaseQ, ist->st->time_base);
    ...
    pkt_dts = av_rescale_q_rnd(pkt.dts, ist->st->time_base, g_TimeBaseQ, (AVRounding)(AV_ROUND_NEAR_INF|AV_ROUND_PASS_MINMAX));
    if ((ist->dec_ctx->codec_type == AVMEDIA_TYPE_VIDEO ||
        ...
        if (delta < -1LL*dts_delta_threshold*AV_TIME_BASE ||
            ...
            pkt.dts -= av_rescale_q(delta, g_TimeBaseQ, ist->st->time_base);
            if (pkt.pts != AV_NOPTS_VALUE)
                pkt.pts -= av_rescale_q(delta, g_TimeBaseQ, ist->st->time_base);
        }
    }
    ...
    if ((ist->dec_ctx->codec_type == AVMEDIA_TYPE_VIDEO ||
        ...
        if (is->iformat->flags & AVFMT_TS_DISCONT) {
            ...
            if (pkt.pts != AV_NOPTS_VALUE){
                int64_t pkt_pts = av_rescale_q(pkt.pts, ist->st->time_base, g_TimeBaseQ);
    ...
    if (pkt.dts != AV_NOPTS_VALUE)
        ifile->last_ts = av_rescale_q(pkt.dts, ist->st->time_base, g_TimeBaseQ);
...
static int transcode(void)
{
    ...
    for (i = 0; i < nb_output_files; i++) {
        ...
        if ((ret = av_write_trailer(os)) < 0) {
            av_log(NULL, AV_LOG_ERROR, "Error writing trailer of %s: %s", os->filename, av_make_error_string(g_szLog, MAX_PATH, ret));
            if (exit_on_error)
                exit_program(1);
        }
    }
